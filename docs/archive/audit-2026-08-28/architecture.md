...
| 🟢 | validation overlap | `src/laivelup/scoring.py:70-130` + `src/laivelup/schema.py:31-51` | Double validation du profil : `schema.validate_profile()` (JSON Schema, structural) + `scoring.normalize_profile()` (business rules, type coercion). Les deux vérifient `pr_sizes`, `retries_after_fact`, `parallel_projects` avec des logiques légèrement différentes. Pas de risque actuel (les deux sont appelés dans `cli.py:_load_profile`), mais divergence silencieuse possible si le schema évolue sans synchroniser normalize. | Documenter la séparation des responsabilités dans une note de code ou ADR. Idéalement, `normalize_profile` ne devrait vérifier que les règles métier non couvertes par le schema. | XS |
