...
def _fix_suggestion(name: str, verdict_level: Level | None, expected_level: str, axis_scores: list) -> str:
    ...
    if errors > 0 and fix:
        ...
        for name, status, detail, verdict, expected_level in results:
            if status == "FAIL" and expected_level:
                print(_fix_suggestion(name, verdict.level if verdict else None,
                                      expected_level, verdict.axis_scores if verdict else []))
...
def main() -> None:
    ...
    parser.add_argument("--expected", type=Path, default=EXPECTED_FILE, help="Chemin vers expected.json")
