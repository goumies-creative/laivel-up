...
class TestDemoExecution:
    ...
    def test_demo_script_syntax(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, "-c", "import ast; ast.parse(open(r'" + str(DEMO_SCRIPT) + "', encoding='utf-8').read())"],
            capture_output=True,
            text=True,
        )
    ...
    def test_demo_importable(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, "-c", "import importlib.util; spec = importlib.util.spec_from_file_location('demo', r'" + str(DEMO_SCRIPT) + "'); mod = importlib.util.module_from_spec(spec); print('OK')"],
            capture_output=True,
            text=True,
            cwd=str(DEMO_DIR),
        )
