...
NIVEAUX_FIXTURES = [
    ...
    (
        ...
        dict(
            traces=full(
                traces_size(['S', 'S']),
                traces_harness(prompts=True),
                traces_intervention(0.9),
                traces_parallel(0),
            )
        ),
    ),
    ...
    (
        ...
        dict(
            traces=full(
                traces_size(['XL', 'XL', 'L']),
                traces_harness(ctx=True, rules=True, loops=True),
                traces_intervention(0.05),
                traces_parallel(3, completed=3),
            )
        ),
    ),
...
@pytest.mark.parametrize(
    'expected,kw', NIVEAUX_FIXTURES, ids=lambda x: x if not isinstance(x, dict) else ''
)
    ...
    verdict = evaluate(
        p(
            traces=full(
                traces_size(['M', 'M']),
                traces_harness(ctx=True),
                traces_intervention(0.8, triangulated=False),
                traces_parallel(1),
            )
        )
    )
...
def test_profil_maison_1_refuse_sur_contradiction():
    ...
    payload = json.loads(
        Path(__file__)
        .parent.parent.joinpath('exemples', 'profil-maison-1.json')
...
def test_profil_maison_2_refuse_sur_donnees_partiales():
    ...
    payload = json.loads(
        Path(__file__)
        .parent.parent.joinpath('exemples', 'profil-maison-2.json')
        .read_text(encoding='utf-8')
    )
